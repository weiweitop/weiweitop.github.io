...
#### 加固态驱动<abbr title="Solid State Drive">SSD</abbr>

先确认一下硬盘位和光驱位的SATA速度，如果光驱位和硬盘位的一样，那SSD随便安在哪儿都可以，但安装在光驱位更简单一些，不用拆卸硬盘。网上有人说SSD安在光驱位休眠后无法唤醒，不过我没有遇到过。

如果只有硬盘位支持SATA 3，安装SSD在这个位置是值得的；并且现在光驱很少使用了，把光驱拆下来放到外置光驱盒做成移动光驱，然后在光驱位安<abbr title="Hard Disk Drive">HDD</abbr>。
