...
参考iFixit的[更换光驱指南](https://www.ifixit.com/Guide/MacBook+Pro+13-Inch+Unibody+Early+2011+Optical+Drive+Replacement/5122){:target="_blank"}，:bangbang:Step 6拔摄像头排线、Step 8拔天线连接头、Step 11拔扬声器接头完全没有必要:bangbang:不要做:exclamation:不要做:exclamation:不要做:exclamation:对待这么柔软纤细的小线线怎么能这么粗鲁:blush:
...
Shift-Option-⌘-R进入Recovery，将Lion恢复安装到SSD，然后依照步骤[升级到Sierra](https://support.apple.com/en-gb/HT208202){:target="_blank"}，最后[升级到High Sierra](https://support.apple.com/en-us/HT208969){:target="_blank"}。
...
还有一种装系统的方法是[创建外部启动盘](https://support.apple.com/kb/HT201372){:target="_blank"}，我觉得Recovery都这么方便了，应该用不上。
...
每个厂商实现TRIM的方式有轻微的不同，大多数厂商并没有做Mac的兼容性测试，有可能造成**数据丢失**或**数据损坏**。可以参考下Linux[内核驱动](https://github.com/torvalds/linux/blob/master/drivers/ata/libata-core.c){:target="_blank"}，下面这些型号的SSD不能很好的支持TRIM。
