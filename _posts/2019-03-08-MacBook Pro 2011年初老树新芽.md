...
Mac越用越慢，菊花转很久才能打开APP。其实我很少用Mac，也没有跟风升级系统，一直稳起还是原配的Lion 10.7，更多的是心理因素吧，哈哈。
...
先确认一下硬盘位和光驱位的SATA速度，如果光驱位和硬盘位的一样，那SSD随便安在哪儿都可以，安装在光驱位更简便一些，不用动到硬盘。网上有人说SSD安在光驱位休眠后无法唤醒，不过我没有遇到过。如果只有硬盘位支持SATA 3，安装SSD在这个位置是值得的；并且现在光驱很少使用了，把光驱拆下来放到外置光驱盒做成移动光驱，然后在光驱位安HDD（Hard Disk Drive）。

幸运的是这台电脑光驱位和硬盘位都支持SATA 3，苹果硬件的冗余确实不错，2011年初的机子都支持SATA 3和WiFi 5G。链接速度的意思是最高支持的速度；协商的链接速度代表实际工作速度，由于外设工作在不同速度下，芯片组向下兼容。从截图中可以看出原配是个SATA 2的HDD。

{% include lightbox.html imgs="SATA_硬盘位.png: SATA硬盘位|SATA_光驱位.png: SATA光驱位" groupname="SATA" %}

参考iFixit的[更换光驱指南](https://www.ifixit.com/Guide/MacBook+Pro+13-Inch+Unibody+Early+2011+Optical+Drive+Replacement/5122)，:bangbang:Step 6拔摄像头排线、Step 8拔天线连接头、Step 11拔扬声器接头完全没有必要:bangbang:不要做:exclamation:不要做:exclamation:不要做:exclamation:对待这么柔软纤细的小线线怎么能这么粗鲁:blush:对了，别忘了买个光驱位硬盘支架。
...
激活TRIM后，每一次删除文件操作系统都发送一个信号到SSD。SSD知道这个文件被删除后，它就能抹去闪存上的数据，为下次写入腾出空间，这样SSD就可以更智能地管理可用空间。

Mac默认只对Apple认证的SSD激活TRIM，对于其他厂商的SSD用户只能借助第三方工具来激活TRIM。之前以为是Apple傲娇，其实是有原因的。Apple规避风险的意识很强，迈过雷区立自己于不败之地:smirk:
...
OS X 10.10.4 Yosemite以后提供了一个新指令强制激活TRIM，打开终端，执行`sudo trimforce enable`，一堆惊悚的警告信息意思是一切都和Apple无关，后果自负。输入**y**，搞定。如果你后悔了，执行`sudo trimforce disable`禁用TRIM。
...
硬件搞定后就该升级操作系统了，HDD上面也没什么数据需要迁移，就打算格式化掉，然后新系统安装在SSD。由于SSD芯片有擦除寿命，计划经常读写的放HDD，经常只读的放SSD？？？
