...
<abbr title="Berkeley Open Infrastructure for Network Computing">BOINC</abbr>平台的主要目的是利用全球个人电脑的闲置运算能力，滴水成海，组成超级计算机。
...
## <abbr title="Search for Extraterrestrial Intelligence">SETI</abbr>

{% include outerlink.html link="https://setiathome.berkeley.edu/" %} 通过使用射电望远镜来监听太空中的窄带无线电信号，期望从浩渺如烟的宇宙信号中搜寻智慧生命发出的信号。有些信号被认为是不可能自然产生的，如果能探测到这样的信号就可以证明地外文明的存在。项目还顺带搜索地外工程的证据，例如戴森球。
...
<abbr title="Radio Frequency Interference">RFI</abbr>通常由电视，广播和卫星发射器产生：
...
- 第一行显示当前运算的进度，主要阶段有：
  ...
  - 快速傅里叶变换（FFT）：望远镜提供的是时域中的数据，就像一条随时间变化的正弦波，我们需要做频谱分析来找到哪些频率的信号较强。这里强烈安利一篇非常有趣的通俗易懂的文章——[傅里叶变换的本质](https://www.cnblogs.com/h2zZhou/p/8405717.html){:target="_blank"}
...
聆听太空的声音[Space Sounds](http://www.spacesounds.com/navigator/navigator.html){:target="_blank"}，可以从[这里](http://www.spacesounds.com/beyond/beyond.html){:target="_blank"}下载桌面版，听听宇宙诞生的啼哭（大爆炸遗迹）、黑洞X射线喷射、船帆座（Vela）脉冲星密集的鼓点、太阳的心跳等非常有趣。但不知道怎么关掉讨厌的背景乐，桌面版的音频文件在解压目录的home\sounds下，可以单独播放出来听。

[从宏观宇宙进入微观宇宙的旅行](http://micro.magnet.fsu.edu/primer/java/scienceopticsu/powersof10/index.html){:target="_blank"}很是震撼，一千万光年外看银河系到亚原子宇宙。

给天文爱好者的链接：[SkyView网络虚拟望远镜](https://skyview.gsfc.nasa.gov/current/cgi/titlepage.pl){:target="_blank"} / [STScI Digitized Sky Survey](http://archive.stsci.edu/cgi-bin/dss_form){:target="_blank"}

这是一个被动搜寻项目，相当于竖起耳朵聆听。<abbr title="Berkeley SETI Research Center">BSRC</abbr>的立场是，在国际社会谨慎考虑过此类行动可能产生的巨大影响和后果之前，不应该故意向已知或未知的地外文明发送信号。（我就想问下先驱者和旅行者探测器携带的地球文明信息是怎么肥事？Arecibo向球状星团M13发射的信息是怎么肥事？虽然雨SETI无瓜）
...
让我们用[德雷克方程](http://www.area52online.com/sections/simulations/drake/guessinggame.htm){:target="_blank"}来预估下星系文明数量，[这里](http://www.area52online.com/sections/simulations/drake/simulation5f.swf){:target="_blank"}是flash文件。
...
{% include outerlink.html link="http://einstein.phys.uwm.edu/" %} 利用LIGO激光干涉引力波天文台，Arecibo射电望远镜和Fermi伽马射线卫星的数据，搜索来自旋转中子星（通常称为脉冲星）的微弱天体物理信号。
...
这个[3D演示软件](https://einsteinathome.org/science/pulsating-science){:target="_blank"}可以探索具有脉冲星的双星系统的多普勒效应。黄线是磁轴，黄锥是沿磁极发射的两个聚焦的无线电波束，白线是自转轴。动画下方的图表显示了观察者（红线）在脉冲星旋转一周期间能测量到的无线电脉冲信号。

{% include lightbox.html imgs="pulsating_science_screenshot.png: 3D演示软件" groupname="演示软件" %}

但是从伽马射线脉冲星中找到周期性的脉冲非常困难，更不要说非常快的毫秒脉冲星。 对于一个典型的脉冲星，Fermi上的<abbr title="Large Area Telescope">LAT</abbr>平均每天仅检测到10个光子，所以必须分析多年的数据来检测周期性。
...
未来计划在太空中建造<abbr title="Laser Interferometer Space Antenna">LISA</abbr>，人类就能探测到10Hz以下的引力波，也就意味着LISA将能够探测到更大质量天体产生的引力波，例如星系中央的超大质量黑洞的合并:+1::+1:。

有趣（无聊）的科学家开发了三个有点儿意思的游戏：

- [Black Hole Hunter](http://www.blackholehunter.org/){:target="_blank"}：在这个在线游戏里先听一段没有干扰的引力波信号，然后从4个有噪音的数据文件中找出包含该引力波信号的那个。反正我是完全听不出来
- [Black Hole Master](https://www.laserlabs.org/blackholemaster.php){:target="_blank"}：类似乒乓球的单机游戏，黑洞是球拍，恒星是乒乓球。利用黑洞的引力和弹弓运动将接近的恒星甩向对家
- [Space Time Quest](https://www.laserlabs.org/spacetimequest.php){:target="_blank"}：在有限的预算下设计自己的引力波探测器，看起来更复杂更有趣，还没时间仔细研究

## Rosetta

{% include outerlink.html link="http://boinc.bakerlab.org/rosetta/" %} 蛋白质的功能以及如何与其他分子相互作用很大程度上取决于其形状（三维结构）。进行蛋白质的三维构造研究，有可能最终发现治愈人类几大疾病的特效药。该项目可以帮助团队加速并拓宽研究思路，从而设计出新的蛋白质以抗击艾滋病、疟疾、癌症以及阿尔茨海默氏症（老年痴呆症）。
...
{% include outerlink.html link="http://www.worldcommunitygrid.org/" %} 推进有关健康，贫困和可持续性的前沿科学研究，包括寻找更有效的治疗艾滋，癌症，登革热以及一些被忽视的热带疾病的方法。
...
该[列表](https://www.worldcommunitygrid.org/research/viewAllProjects.do?proj=comp){:target="_blank"}显示了已经完成的历史研究项目。

## Quake-Catcher Network

{% include outerlink.html link="http://quakecatcher.net/sensor/" %} 地震捕捉网络使用连接到计算机和智能手机的传感器来检测地震波。截图显示的是最近发生在内江的一次地震。

{% include lightbox.html imgs="Quake_Catcher.png: 检测地震波" groupname="Quake Catcher" %}

## Asteroids

{% include outerlink.html link="http://asteroidsathome.net/boinc/" %} 太阳系包含几十万颗小行星，而且每天都新发现数百个。对大部分来说，我们只知道这个小行星的大小，对其他物理参数比如形状、自传周期、自传轴方向则一无所知。
...
## <abbr title="Chinese Academy of Sciences">CAS</abbr>

{% include outerlink.html link="http://casathome.ihep.ac.cn/" %} 由中科院高能物理研究所电子计算中心主办，主要的研究项目有：
...
{% include outerlink.html link="http://www.cosmologyathome.org/" %} 通过搜索一系列与最前沿的宇宙学和粒子物理学数据相符的理论模型来更好地理解我们的宇宙。
...
{% include outerlink.html link="http://denis.usj.es/denisathome/" %} 通过心脏电生理模拟，研究心脏细胞的电活动。
...
{% include outerlink.html link="http://www.gpugrid.net/" %} 致力于生物医学研究，通过NVIDIA显卡（GPU）进行分子模拟来了解蛋白质在健康和疾病中的功能。
...
## <abbr title="Large Hadron Collider">LHC</abbr>

{% include outerlink.html link="https://lhcathome.cern.ch/lhcathome/" %} 通过模拟加速器并帮助改进LHC及其探测器的设计来推进粒子和加速器物理学

{% include lightbox.html imgs="LHC.jpg: LHC" groupname="LHC" %}

## Milyway

{% include outerlink.html link="http://milkyway.cs.rpi.edu/milkyway/" %} 通过分析银河系银晕中的恒星来研究银河系的历史，这个项目也包括搜寻难以捉摸的暗物质。
...
这篇[科普文章](https://milkyway.cs.rpi.edu/milkyway/science.php){:target="_blank"}有详细介绍。

## RNA World

{% include outerlink.html link="http://www.rnaworld.de/rnaworld/" %} 该项目的目标是系统地识别迄今为止所有已知生物的所有已知RNA家族成员。
...
{% include outerlink.html link="https://universeathome.pl/universe/" %} 该项目模拟数量众多的恒星演化。由于整个星系是由数十亿颗恒星组成的，通过这种方式我们就能够重现这种恒星种群的演化。所获得的广阔画面使科学家可以将结果与**所有**可获得的观测数据进行比较。如此大规模的比较远胜于在单个系统上进行。对整个恒星种群的调查不仅为我们提供了其演化的信息，还提供了多样性和统计的信息。这种方法称为种群综合分析。
...
http://www.einstein-online.info/  EXCELLENT
Space Time Quest
