...
创建一个简单对象还比较容易精确测量，销毁一个对象的时间就没那么容易精确测量了。因为JVM屏蔽了底层细节，程序员并不知道它什么时候真正的销毁对象并回收资源。你可以提建议，但什么时候听就看我心情了:sunglasses:

想了一下，大概有这两条路：

* 从JVM入手。这应该是我所能想到的最精确的方法，比如在对象创建后或销毁后JVM有个Callback什么的。以上纯属YY:joy:，如有雷同，绝非巧合。时间、精力、脑力有限，没去近一步研究。
...
最近刚好看到`java.lang.ref`包，`PhantomReference`代表最接近生命周期终点的*phantom reachable*，当GC认为referent，也就是我们的简单对象，变成*phantom reachable*的时候，它就会将该*reference*添加到*queue*中。
...
这次的结果也差不太多：`3421+4973193=4973193 ns`
...
以这个版本为蓝本，测量一下创建一个简单对象，最快需要多少时间。我们先注释掉销毁对象的部分，然后运行的时候加`-Xcomp`参数，得到的结果是`21 ns`

#### V4

这个版本改变成创建一个对象销毁一个的方式。
