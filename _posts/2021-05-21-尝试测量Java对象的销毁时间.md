...
public class WithoutFinalizationV1 {
    ...
    public static void main(String[] args) {
        ...
        long time = System.nanoTime() - start;//only need this time point
        System.out.println("WithoutV1 "+waitCount+" "+time);
    }
}
{% endhighlight %}
...
public class WithFinalizationV1 {
    ...
    public static void main(String[] args) throws Exception {
        ReferenceQueue<WithFinalizationV1> queue = new ReferenceQueue<>();
        long start = System.nanoTime();
        
        PhantomReference<WithFinalizationV1> rf = new PhantomReference<>(
                new WithFinalizationV1(), queue);
        System.gc();//advise JVM to do GC
        synchronized(lock){
            while(!finalized){
                lock.wait();
        ...
        System.out.println("WithV1 "+waitCount +" "+time);
    }
}
{% endhighlight %}
...
GC后，finalizable类的实例会在另一个queue里等待`java.lang.ref.Finalizer`调用它的`finalize()`方法；调用完成后，该实例变成unreachable的。直到下次GC才会进入我们的queue里。

下次调用`gc()`的时机通过wait-notify机制实现，这样就不可避免有额外的开销。最好记录是`11136119 ns`，耗时大概是上面版本的两倍。

另一个实现方式是`Thread.sleep(x)`，但这个`x`就不好拿捏了。太大，测量误差更大；太小，该次GC发生在`Finalizer`调用实例的`finalize()`方法之前，这个实例就永远没法enqueue，我们的测试程序的就活性失败了。`x`是1的话，这两种方式运行的结果都差不多，没有数量级上的差别。
...
public class WithFinalizationV2 {
    ...
    public static void main(String[] args) throws Exception {
        ReferenceQueue<WithFinalizationV2> queue = new ReferenceQueue<>();
        long createTime;
        long destroyTime;
        ...
        PhantomReference<WithFinalizationV2> rf = new PhantomReference<>(o, queue);
        Object x = null;
        int waitCount = -1;
        o = null;//explicitly clear reference
        ...
        System.out.println("WithV2 "+waitCount 
                +" "+createTime +" "+ destroyTime);
    }
}
...
最快结果是：`6415+12250587=12257002`，是上面版本的2.5倍左右。

#### V3 - NG

遇到构造大量finalizable类的实例，这个机制就不工作的问题。研究中。。。
{% comment %}
...
有`finalize()`方法类的实例需要额外的一道手续才能被回收，显然比回收普通类的实例更加耗时。
