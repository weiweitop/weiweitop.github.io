...
Label和Title前缀是可以自定义的，默认配置是`prefix: '[Vssue]', labels: ['Vssue']`，那我们可以定义一个更通范的名字让Issue可以和其它评论系统共享，比如[Gitalk](#gitalk)。注意，Label不要有空格，不然Vssue找不到Issue。更多配置请参见[这里](https://vssue.js.org/options/)。
