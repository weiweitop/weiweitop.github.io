...
# [Staticman V2](https://staticman.net/)
...
- 评论无法分页，除非Jekyll支持，比如给每一篇文章的每一页评论生成一个静态文件。突然想到个假分页（前端一次性load所有评论）的方法：构造个json文件包含所有的评论，JavaScript load进来就可以做分页了，由于每次提交都会触发GitHub Pages重新生成文件，所以json肯定包含新加的评论。
...
{% include lightbox.html src="gitalk_paging.png" groupname="gitalk" title="分页加载评论" %}

评论列表动画：

{% include lightbox.html src="gitalk_list.png" groupname="gitalk" title="评论列表动画" %}

支持Markdown语法，全屏遮罩效果，太酷了:+1:：

{% include lightbox.html src="gitalk_comment.png" groupname="gitalk" title="评论框" %}

评论预览：

{% include lightbox.html src="gitalk_preview.png" groupname="gitalk" title="评论预览" %}
