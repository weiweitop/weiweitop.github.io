...
最厉害的是HackMii，当它运行的时候，什么软改环境都没有，一切靠自己:+1:
...
`BootMii as boot2`利用boot1的漏洞直接注入一段程序到boot2中，替换掉原来的Loader程序，这样我们就能运行其它程序而不是boot2了。注意它运行的时间点，在第三步的最开始，也就是在加载任何IOS之前，在读取NAND文件系统之前，当然也就在显示系统菜单之前，所以就能在系统启动初始阶段上下其手了:+1:
...
- cIOS236（基于IOS36），安装器是IOS236_Installer_v6，把任天堂修复的Bug又恢复回来，这就是著名的FakeSign Bug (Trucha bug / Signing bug / Strncmp bug) 。有了这个Bug，用户可以轻松绕过任天堂的安全检查而安装自制APP。造成这个Bug的原因是任天堂没有正确使用RSA数字签名验证，一是用比较字符串的方法来比较二进制数据:sweat:，还有就是没有检查padding，这个都不能说是Bug，算是一个事故吧。再次请注意，这个Bug存在于**IOS**早期版本中。老版本的Installer把补丁直接打在了slot36，新版本的才分开安装到了slot236以保持原生IOS的干净，**已经过时，不需要了**。
