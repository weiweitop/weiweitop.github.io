...
HackMii同样利用`boot1`中的FakeSign Bug来达成`BootMii as boot2`，后来%TODO%

完全运行在不同层次，就像大刘球状闪电中的丁仪一样，大家都聚焦在用闪电生成球状闪电时，而他已经思考到了另一个层次。
...
`BootMii as boot2`利用boot1的漏洞% strncmp bug in boot1%直接注入一段程序到boot2中，替换掉原来的Loader程序，这样我们就能运行其它程序而不是boot2了。注意它运行的时间点，在第三步的最开始，也就是在加载任何IOS之前，在读取NAND文件系统之前，当然也就在显示系统菜单之前，所以就能在系统启动初始阶段上下其手了:+1:

因为boot2有大小限制，所以用到的一些文件就写入到了SD卡，也就是上面的`prepare SD card`。

美好的事物总是不长久，任天堂知道了这个漏洞，以2008年的某个时间点为分界线，之后出厂的Wii堵住了boot1能运行修改后的boot2的漏洞。之后的Wii就只能安装成`BootMii as IOS`了（slot 254），失去了底层防变砖的功能。前面说过boot1相当于只读的，所以`BootMii as boot2`的总是可以`BootMii as boot2`，`BootMii as IOS`的一直是`BootMii as IOS`。

那么当初在电玩店买Wii的时候，老板儿不会主动给你写有BootMii文件的SD卡，你当时可能也不知道有这个东西，没有这些文件不管安装方式是`BootMii as boot2`还是`BootMii as IOS`，BootMii都无法运行，这也就是安装IOS58之前没有备份NAND的原因。
...
- cIOS236（基于IOS36），安装器是IOS236_Installer_v6，把任天堂修复的Bug又恢复回来，这就是著名的FakeSign Bug (Trucha bug / Signing bug / Strncmp bug) 。有了这个Bug，用户可以轻松绕过任天堂的安全检查而安装自制APP。造成这个Bug的原因是任天堂没有正确使用RSA数字签名验证，一是用比较字符串的方法来比较二进制数据:sweat:，还有就是没有检查padding，这个都不能说是Bug，算是一个事故了。再次请注意，这个Bug存在于**IOS**早期版本中。老版本的Installer把补丁直接打在了slot36，新版本的才分开安装到了slot236以保持原生IOS的干净，**已经过时，不需要了**。
...
先只安装cIOS249和cIOS250就够了，不建议做cIOS补全（这个不是收集卡牌），操作越多，错误机会越大。除非确实不兼容你想玩的游戏，不过这个游戏也许永远也玩不到。不要做不需要做的步骤！

其实你可以安装到200+的任意slot，但如果不用约定俗成的slot的话，Game Loader里面菜单可能就没有你自己选的slot:joy:
