...
`BootMii as boot2`利用boot1的漏洞直接注入一段程序到boot2中，替换掉原来的Loader程序，这样我们就能在boot2之前运行HBC了。注意它运行的时间点，在第三步的最开始，也就是在加载任何IOS之前，在读取NAND文件系统之前，当然也就在显示系统菜单之前，所以就能在系统启动初始阶段上下其手了:+1:
...
- cIOS236（基于IOS36），安装器是IOS236_Installer_v6，把任天堂修复的Bug又恢复回来，这就是著名的FakeSign Bug (Trucha bug / Signing bug / Strncmp bug) 。有了这个Bug，用户可以轻松绕过任天堂的安全检查而安装自制APP。造成这个Bug的原因是任天堂没有正确使用RSA数字签名验证，一是用比较字符串的方法来比较二进制数据，还有就是没有检查padding，这个都不能说是Bug，算一次事故吧。老版本的Installer直接安装在了slot36，新版本的才分开安装到了slot236，**已经过时，不需要了**。
...
先只安装cIOS249和cIOS250就够了，不建议做cIOS补全（这个不是收集卡牌），操作越多，错误机会越大。除非确实不兼容你想玩的游戏，不过这个游戏也许永远也玩不到。
