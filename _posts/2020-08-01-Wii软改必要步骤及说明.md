...
BootMii最主要的作用就是备份和恢复512MB的机身内存NAND。就像系统镜像一样，如果后面不小心搞砸了，还有后悔药可以吃。强烈建议至少备份一次，防变砖保护之一！

说到BootMii，无法绕开的就是Wii的启动流程。在显示系统菜单之前，Wii进行了一系列启动流程：

1. boot0：一个ROM中的程序，加载boot1并且计算它的SHA1值，然后与OTP中的值比较，如果不一样，系统将停止启动。很显然，OTP中的值是在出厂前写入的。由于boot0是在ROM中所以无法修改。
2. boot1：一个NAND中的程序，加载boot2
